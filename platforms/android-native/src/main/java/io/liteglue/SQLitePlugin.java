...
public class SQLitePlugin extends ReactContextBaseJavaModule {
     ...
     * @param dbname   The name of the database file
     */
    private SQLiteAndroidDatabase openDatabase(String dbname, String key, String assetFilePath, int openFlags, CallbackContext cbc, boolean old_impl) throws Exception {
        InputStream in = null;
        File dbfile = null;
        try {
            ...
            if (assetImportRequested) {
                if (assetFilePath.compareTo("1") == 0) {
                ...
                } else if (assetFilePath.toLowerCase().contains("database")){
                    File databaseFile = this.getContext().getDatabasePath(dbname);
                    ...
                    try {
                        ...
                        assetImportError = true;
                        FLog.e(TAG, "Error opening pre-populated DB asset in app bundle database subdirectory: " + assetFilePath);
                    }
                } else {
                    ...
                    try {
                        ...
                        assetImportError = true;
                        FLog.e(TAG, "Error opening pre-populated DB asset in app bundle file subdirectory: " + assetFilePath);
                    }
                }
            }
            ...
            if (key != null && !key.trim().isEmpty()) {
                mydb.open(dbfile, key, openFlags);
            } else {
                mydb.open(dbfile, openFlags);
            }

//            mydb.mydb = mydb.theDB;
            if (cbc != null)
    ...
    private class DBRunner implements Runnable {
        ...
        DBRunner(final String dbname, ReadableMap options, CallbackContext cbc) {
            ...
            try {
                ...
                if (this.assetFilename != null && this.assetFilename.length() > 0) {
                    ...
                    key1 = SQLitePluginConverter.getString(options,"key",null);
                }
            } catch (Exception ex){
        ...
        public void run() {
            try {
                this.mydb = openDatabase(dbname, key, this.assetFilename, this.openFlags, this.openCbc, this.oldImpl);
            } catch (Exception ex) {
            ...
            try {
                ...
                while (!dbq.stop) {
                    System.out.println(dbq.queries);
                    System.out.println(dbq.queryParams);
                    System.out.println(dbq.queryIDs);
