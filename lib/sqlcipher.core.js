...
var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, SQLCipherFactory, SQLCipherPlugin, SQLCipherPluginTransaction, argsArray, dblocations, newSQLError, root, txLocks;
...
plugin.exec = function(method, options, success, error) {
  if (plugin.sqlcipherPlugin.DEBUG){
    plugin.log('SQLCipher.' + method + '(' + JSON.stringify(options) + ')');
  }
...
SQLCipherPlugin = function(openargs, openSuccess, openError) {
  ...
  if (typeof dbname !== 'string') {
    throw newSQLError('sqlcipher plugin database name must be a string');
  }
...
SQLCipherPlugin.prototype.databaseFeatures = {
  isSQLCipherPluginDatabase: true
};
...
SQLCipherPlugin.prototype.transaction = function(fn, error, success) {
  ...
  this.addTransaction(new SQLCipherPluginTransaction(this, fn, error, success, true, false));
};
...
SQLCipherPlugin.prototype.readTransaction = function(fn, error, success) {
  ...
  this.addTransaction(new SQLCipherPluginTransaction(this, fn, error, success, false, true));
};
...
SQLCipherPlugin.prototype.sqlBatch = function(sqlStatements, success, error) {
  ...
  this.addTransaction(new SQLCipherPluginTransaction(this, myfn, myerror, mysuccess, true, false));
};
...
SQLCipherPlugin.prototype.executeSql = function(statement, params, success, error) {
  ...
  this.addTransaction(new SQLCipherPluginTransaction(this, myfn, null, null, false, false));
};

SQLCipherPluginTransaction = function(db, fn, error, success, txlock, readOnly) {
...
SQLCipherPluginTransaction.prototype.start = function() {
  var err;
  try {
    this.fn(this);
    this.run();
...
SQLCipherPluginTransaction.prototype.executeSql = function(sql, values, success, error) {
  var that = this;
  if (that.finalized) {
    throw {
...
SQLCipherPluginTransaction.prototype.addStatement = function(sql, values, success, error) {
...
SQLCipherPluginTransaction.prototype.handleStatementSuccess = function(handler, response) {
...
SQLCipherPluginTransaction.prototype.handleStatementFailure = function(handler, response) {
  if (!handler) {
...
SQLCipherPluginTransaction.prototype.run = function() {
...
SQLCipherPluginTransaction.prototype.abort = function(txFailure) {
...
SQLCipherPluginTransaction.prototype.finish = function() {
  var failed, succeeded, tx;
  if (this.finalized) {
    return;
  }
...
SQLCipherPluginTransaction.prototype.abortFromQ = function(sqlerror) {
  if (this.error) {
    this.error(sqlerror);
  }
};
...
SQLCipherFactory = function(){};

SQLCipherFactory.prototype.DEBUG = function(debug) {
  plugin.log("Setting debug to:",debug);
  plugin.sqlcipherPlugin.DEBUG = debug;
};

SQLCipherFactory.prototype.sqlcipherFeatures = function() {
  return {
    isSQLCipherPlugin: true
  };
};

SQLCipherFactory.prototype.openDatabase = argsArray(function(args) {
...
SQLCipherFactory.prototype.echoTest = function(success, error) {
...
SQLCipherFactory.prototype.deleteDatabase = function(first,success, error) {
  var args = {};
  if (first.constructor === String) {
    args.path = first;
...
plugin.sqlcipherPlugin = {
  SQLCipherFactory : SQLCipherFactory,
  SQLCipherPluginTransaction : SQLCipherPluginTransaction,
  SQLCipherPlugin : SQLCipherPlugin,
  log: plugin.log
};

module.exports = plugin.sqlcipherPlugin;
